...
static void SetSysClockTo72(void)
{
    __IO uint32_t tmp_StartUpCounter = 0;
    __IO uint32_t tmp_HSEStatus = 0;
    __IO uint32_t tmp_HSIStatus = 0;
    ...
    else
    {
        ...
        configuration. To deal with the error,the user can add here some code  */
			 /* If HSE fails to start, the application will have wrong clock
          configuration. To deal with the error,the user can add here some code  */
        /* Wait untill HSI is ready ,but exit if Time out is reached */
        RCC->GCCR &= ~((uint32_t)RCC_GCCR_HSEEN);
        RCC->GCCR |= ((uint32_t)RCC_GCCR_HSIEN);
        tmp_StartUpCounter = 0;
        do
        {
            tmp_HSIStatus = RCC->GCCR & RCC_GCCR_HSISTB;
            tmp_StartUpCounter++;
        }
        while((tmp_HSEStatus == 0) && (tmp_StartUpCounter != HSI_STARTUP_TIMEOUT));
        if ((RCC->GCCR & RCC_GCCR_HSISTB) != RESET)
        {
            tmp_HSIStatus = (uint32_t)0x01;
        }
        else
        {
            tmp_HSIStatus = (uint32_t)0x00;
        }
        if (tmp_HSIStatus == (uint32_t)0x01)
        {
            /* AHB = SYSCLK not divided */
            RCC->GCFGR |= (uint32_t)RCC_GCFGR_AHBPS_DIV1;
